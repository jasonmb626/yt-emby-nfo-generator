...
out_path = os.path.join(out_base_path, playlist_data['channel_name'], f"Season {season_nbr:02d}")
if not os.path.exists(out_path):
    os.makedirs(out_path)

nfo_data = SEASON_NFO_TEMPLATE.replace('%TITLE%', playlist_data['playlist_name']) \
...
for file in files:
    ...
    nfo_data = VIDEO_NFO_TEMPLATE.replace('%TITLE%', video_data['video_title']) \
